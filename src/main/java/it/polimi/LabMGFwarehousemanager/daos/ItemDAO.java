...
public class ItemDAO {
    ...
    private String getAvailableToolsByTimeRange = "SELECT * FROM items WHERE type = 0" +
            ...
            "                                        WHERE r.status <2 AND ((? BETWEEN r.start AND r.end)" +
    ...
    private String getAccessoriesQuery = "SELECT * FROM items WHERE type = 1";
    public ArrayList<ItemBean> getAccessories() throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement(getAccessoriesQuery);
        ResultSet resultSet = preparedStatement.executeQuery();
        ArrayList<ItemBean> itemBeans = new ArrayList<>();
        while (resultSet.next()){
            itemBeans.add(createItemBeanFromResultSet(resultSet));
    ...
    private String getAllDocumentsQuery = "SELECT * FROM documents WHERE NOT type = 4";
    public ArrayList<DocumentBean> getAllDocuments() throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement(getAllDocumentsQuery);
        ResultSet resultSet = preparedStatement.executeQuery();
    ...
    public ArrayList<Map<String, String>> getUnavailablePeriods(ArrayList<Integer> items) throws SQLException {
        ...
        queryBuilder.append("WHERE r.status < 2 AND r.end >= NOW() AND ");
        ...
        while (resultSet.next()) {
            ...
            period.put("start", resultSet.getTimestamp("start").toString().substring(0, 10));
            period.put("end", LocalDate.parse(resultSet.getTimestamp("end").toString().substring(0,10)).plusDays(1).toString());
            periods.add(period);
        }
    ...
    private final String getDocumentPathByIdQuery = "SELECT path FROM documents WHERE id = ?";
    public String getDocumentPathById(int documentId) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(getDocumentPathByIdQuery);
    ...
    private final String editDocumentByIdQuery = "UPDATE documents SET name = ?, type = ?, language = ? WHERE id = ?";
    public void updateDocumentById(int id, String name, int type, int language) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement(editDocumentByIdQuery);
        preparedStatement.setString(1, name);
